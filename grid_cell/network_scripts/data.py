...
class Dataset(torch.utils.data.Dataset):
	...
	Inputs:
		...
		labels_x: vector of ground truth x locations
		labels_y: vector of ground truth y locations
	...
	'''
	def __init__(self, device, cochains, labels_x, labels_y):
		self.cochains = cochains
		self.labels_x = labels_x
		self.labels_y = labels_y
		self.labels = np.zeros((2, labels_x.size))
		self.labels[0, :] = self.labels_x
		self.labels[1, :] = self.labels_y
		self.device = device

	def __len__(self):
		return int(len(self.labels_x))
...
class Dataset_gen(torch.utils.data.Dataset):
	...
	Returns: 
		...
		label:  ground truth label used for computing loss
	'''
	def __init__(self, device, cochains, labels_x, labels_y, intervals_per_sample):
		self.cochains = cochains
		self.labels_x = labels_x
		self.labels_y = labels_y
		self.labels = np.zeros((2, labels_x.size))
...
class DatasetSCRNN(torch.utils.data.Dataset):
	...
	Returns: 
		...
		label:  ground truth label used for computing loss 
	'''
	def __init__(self, device, cochains, labels_x, labels_y, sequence_length):
		self.device = device
		self.cochains = cochains
		self.labels_x = labels_x
		self.labels_y = labels_y
	...
	def __len__(self):
		return int(len(self.labels_x)) - self.seq_length + 1
...
class DatasetFFNN(torch.utils.data.Dataset):
	...
	Returns: 
		...
		label:  ground truth label used for computing loss 
	'''
	def __init__(self, device, spike_count_matrix, labels_x, labels_y):
		...
		evens = [2*k for k in range(labels_x.size)]
		odds = [2*k + 1 for k in range(labels_x.size)]
		self.labels[evens] = self.labels_x
		self.labels[odds] = self.labels_y

	def __len__(self):
		return len(self.labels_x)

	def __getitem__(self, idx):
		sample = torch.tensor(self.spike_count_matrix[:,idx], device=self.device)
		label = torch.tensor(self.labels[idx*2:(idx+1)*2], dtype=torch.float32).squeeze()
...
class DatasetRNN(torch.utils.data.Dataset):
	...
	Returns: 
		...
		label:  ground truth label used for computing loss
	'''
	def __init__(self, device, spike_count_matrix, labels_x, labels_y, sequence_length):
		self.device = device
		self.spike_count_matrix = spike_count_matrix
		self.labels_x = labels_x
		self.labels_y = labels_y
